...
function ahcfree_savesettings() {
    ...
    $posts_type = '';
    $set_ips = sanitize_text_field($_POST['set_ips']);
    $set_google_map = '';
...
function ahcfree_get_vsitors_by_country($all,$cnt=true,$start='',$limit='',$fdt='',$tdt='') {
    ...
    {
		$fdt = sanitize_text_field($_POST['t_from_dt']);
		$tdt = sanitize_text_field($_POST['t_to_dt']);
	}
...
function ahcfree_get_recent_visitors($all,$cnt=true,$start='',$limit='',$fdt='',$tdt='',$ip='') {
    ...
    { 
		$fdt = sanitize_text_field($_POST['r_from_dt']);
		$tdt = sanitize_text_field($_POST['r_to_dt']);
	}
	...
	{
		$ip = sanitize_text_field($_POST['ip_addr']); 	
	}
...
function ahcfree_get_latest_search_key_words_used($all,$cnt=true,$start='',$limit='',$fdt='',$tdt='') {
    ...
    { 
		$fdt = sanitize_text_field($_POST['from_dt']);
		$tdt = sanitize_text_field($_POST['to_dt']);	 
	} 
...
function ahcfree_track_visitor()
{
    ...
    if (!ahcfree_is_login_page() && !ahcfree_is_search_engine_bot() && !ahcfree_is_wordpress_bot()) {
        if (!in_array($client_ip_address, $exclude_ips_arr)) {
            ...
            $_SERVER['HTTP_REFERER'] = sanitize_text_field($_POST['referer']);
            $_SERVER['HTTP_USER_AGENT'] = sanitize_text_field($_POST['useragent']);
            $_SERVER['SERVER_NAME'] = sanitize_text_field($_POST['servername']);
            $_SERVER['HTTP_HOST'] = sanitize_text_field($_POST['hostname']);
            $_SERVER['REQUEST_URI'] = sanitize_text_field($_POST['request_uri']);
...
wp_register_style( 'bootstrap_css', 'https://maxcdn.bootstrapcdn.com/bootstrap/3.2.0/css/bootstrap.min.css' );
	wp_enqueue_style('bootstrap_css');

	wp_register_script( 'bootstrap_js', 'https://maxcdn.bootstrapcdn.com/bootstrap/3.2.0/js/bootstrap.min.js', null, null, true );
	wp_enqueue_script('bootstrap_js');

	wp_register_script( 'slimselect_js', 'https://cdnjs.cloudflare.com/ajax/libs/slim-select/1.25.0/slimselect.min.js', null, null, true );
	wp_enqueue_script('slimselect_js');


	wp_register_style( 'slimselect', 'https://cdnjs.cloudflare.com/ajax/libs/slim-select/1.25.0/slimselect.min.css' );
...
function ahcfree_settings_scripts() {
	...
	wp_register_script( 'bootstrap_js', 'https://maxcdn.bootstrapcdn.com/bootstrap/3.2.0/js/bootstrap.min.js', array(), '1.0.0', true );
	wp_enqueue_script('bootstrap_js');

	wp_register_script( 'slimselect_js', 'https://cdnjs.cloudflare.com/ajax/libs/slim-select/1.25.0/slimselect.min.js', array(), '1.0.0', true );
	wp_enqueue_script('slimselect_js');
...
function ahcfree_get_hits_by_custom_duration_callback(){
    $hits_duration = sanitize_text_field($_POST['hits_duration']);
    $custom_timezone_offset = ahcfree_get_current_timezone_offset();
    $custom_timezone = new DateTimeZone(ahcfree_get_timezone_string());
